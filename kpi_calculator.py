...
def _prepare_throughput_data(ticket_data_df, kpi_targets_df):
    ...
    grouped = (
        ...
        .agg(
            ...
            TotalHours=('TimespentSeconds', lambda x: x.sum() / ProcessingConfig.SECONDS_TO_HOURS)  # Convert seconds to hours
        )
        .reset_index()
    )
    
    # Step 3: Filter kpi_targets_df so that the column 'KPI' only includes these values Lead Cycle Response
    kpi_targets_df = kpi_targets_df[kpi_targets_df['KPI'].isin(BusinessRules.THROUGHPUT_KPI_TYPES)]
...
def _add_throughput_columns(result):
    ...
    result['KPI Value'] = result.apply(_calculate_preferred_throughput, axis=1)
...
def _merge_kpi_targets_and_finalize(result, ticket_resolution_times, kpi_targets_df):
    ...
    kpi_targets_df = kpi_targets_df[kpi_targets_df['KPI'].isin(BusinessRules.TIME_BASED_KPI_TYPES)]
...
def get_issue_rate(df: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    grouped['issues_percentage'] = (grouped['issues_count'] / grouped['total_tickets']) * PowerBIConfig.PERCENTAGE_MULTIPLIER
